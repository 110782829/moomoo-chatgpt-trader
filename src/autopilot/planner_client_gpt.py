...
- A "history" map may list per-symbol {"last_action","realized_r"}; favor add/hold when realized_r > 0 and avoid fresh opens when realized_r < 0. "recent_decisions_map" summarises prior planner verdicts per symbol (status: kept/rejected/guardrail/etc); avoid repeating a rejected action unless something material changed. "eval_feedback" lists recent evaluator scores (score + keep boolean); aim for scores ≥ threshold (~0.25).
